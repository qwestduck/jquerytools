...
(function ($) {
    ...
    */
    ...
    tool.addFormatter('default', function (text, date, flags, lang) {
        return text.replace(/d{1,4}|m{1,4}|yy(?:yy)?|"\w*"|'\w*'/g, function ($0) {
            if (typeof flags[$0] !== "undefined") {
                return flags[$0];
            } else {
                return $0;
            }
    ...
    tool.addFormatter('prefixed', function (text, date, flags, lang) {
        return text.replace(/%(d{1,4}|m{1,4}|yy(?:yy)?|"[\w]*"|'[\w]*')/g, function ($0, $1) {
            if (typeof flags[$1] !== "undefined") {
                return flags[$1];
            } else {
                return $0;
            }
...
//}}}
    ...
    function Dateinput(input, conf) {
        ...
        if (input.attr("type") === 'date') {
            ...
            if (conf.value) {
                clone.val(conf.value);   // jquery 1.6.2 val(undefined) will clear val()
            }
        ...
        $.extend(self, {
            ...
            setValue: function (year, month, day) {
                ...
                // variables
                tmp = new Date(year, month, 1 - conf.firstDay);
                begin = tmp.getDay();
                ...
                if (conf.selectors) {
                    ...
                    for (i = yearNow + conf.yearRange[0];  i < yearNow + conf.yearRange[1]; i = i + 1) {
                ...
                // !begin === "sunday"
                for (j = !begin ? -7 : 0; j < (!begin ? 35 : 42); j = j + 1) {
                    a = $("<a/>");
    ...
    $.fn.dateinput = function (conf) {
        ...
        this.each(function () {
            var el = new Dateinput($(this), conf),
                input;
            instances.push(el);
            input = el.getInput().data("dateinput", el);
