...
(function ($) {
    ...
    loadEffect = function (pos, onLoad) {
        ...
        if (!img) {
            bg = overlay.css("backgroundImage");
        ...
        // initial top & left
        itop = conf.start.top || Math.round(w.height() / 2);
        ileft = conf.start.left || Math.round(w.width() / 2);

        if (trigger) {
            p = getPosition(trigger);
            itop = p.top;
            ileft = p.left;
        }
        ...
        // begin growing
        img.animate(
            {top: pos.top, left: pos.left, width: oWidth},
            conf.speed,
            function () {
